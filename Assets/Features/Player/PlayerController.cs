...
public class PlayerController : MonoBehaviour
{
    ...
    public float reloadLaserTime = 5f;
    ...
    private float lastFireLaserTime = -10;
    ...
    public void InitPlayerEvents()
    {
        ...
        _playerEvents.Add(new EventConfig(Abilities.BindableReason.FireButtonPressed,
            TypeEvent.Down,
            fireKeyCode));
        _playerEvents.Add(new EventConfig(Abilities.BindableReason.LaserButtonPressed,
            TypeEvent.Down,
            laserKeyCode));
    ...
    public void Laser()
    {
        if (Time.time > lastFireLaserTime + reloadLaserTime)
        {
            laser = Instantiate(LaserPrefab);

            laser.transform.position = new Vector2(transform.position.x + 1.5f * _direction, transform.position.y);
            laser.transform.parent = gameObject.transform;
            laser.Setup("Player", "Enemy", _direction);
